...
def compute_sheaf_laplacian(
    ...
    Mathematical Background:
        ...
        3. Key properties:
           ...
           - Eigenvalues measure "frequencies" of inconsistency
    
    Real-World Interpretation:
        
        Think of the Laplacian as encoding "how information flows":
        - Zero eigenvalue: Consensus mode (everyone agrees)
        - Small eigenvalues: Near-consensus modes
        - Large eigenvalues: High-disagreement modes
    ...
    Returns:
        The sheaf Laplacian matrix (n*d × n*d where n=modalities, d=dimension)
...
def diffuse_to_consensus(
    ...
    Mathematical Background:
        ...
        The heat equation dx/dt = -Lx has solution:
            x(t) = e^{-Lt} x(0)
        
        As t → ∞:
            x(∞) = projection of x(0) onto ker(L)
        
        This is the "consensus" — the closest globally consistent state.
        
        Properties:
        - Minimizes total disagreement: xᵀLx
        - Preserves total "mass": Σᵢ xᵢ (for standard Laplacian)
        - Converges exponentially fast (rate = smallest nonzero eigenvalue)
    
    Real-World Interpretation:
        
        Think of it as "heat flow" or "opinion dynamics":
        - Each node adjusts toward its neighbors
        - Disagreements smooth out over time
        - Equilibrium = everyone agrees (consensus)
        
        Example: Sensor fusion
        - Sensors: 20°C, 21°C, 22°C (inconsistent)
        - After diffusion: 21°C, 21°C, 21°C (consensus)
    
    Args:
        graph: The modality graph
        embeddings: Initial embeddings (possibly inconsistent)
        num_steps: Number of diffusion steps (more = closer to consensus)
        step_size: Step size for Euler integration (smaller = more stable)
    ...
    Example:
        ...
        ...     'sensor_b': np.array([22.0]),
        ... }
        >>> 
        >>> # Diffuse to consensus
        >>> consensus = diffuse_to_consensus(graph, embeddings, num_steps=100)
        >>> # consensus ≈ {'sensor_a': [21.0], 'sensor_b': [21.0]}
    """
    modalities = graph.modalities
    n = len(modalities)
    d = len(next(iter(embeddings.values())).flatten())
